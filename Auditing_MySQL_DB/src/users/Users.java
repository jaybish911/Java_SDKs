...
// ONLY SECTION TO UPDATE FOR NEW SYSTEMS
		...
		File users_log = new File("C:\\Auditing_SDKs\\43FedUATAuditSDKs_AWS\\SDKlogs\\users_log.txt");
...
//Total Users Pulled from BO
		int UsersSelectedfromCMS = boInfoObjects.size();
		
//Arrays for User Details and User Aliases
...
//Counters for User Details and User Aliases
		int UserCntr = 0;
		int UACntr= 0;
		
		// USER DETAILS MODULE
		// write to users_log file
		outputStream.println("## User Details Module Log File ##");
		outputStream.flush();
...
// USER ALIASES MODULE
			...
			try {
				...
				for (Iterator boAliases = boUserAccount.getAliases().iterator(); boAliases.hasNext();) {
					...
					// write to log file
					outputStream.println(" - "+AliasName);
					outputStream.flush();
		...
		// write to log file
		outputStream.println(new SimpleDateFormat("MM/dd/yyyy HH:mm:ss").format(new Date()) + "-> "
				+ "User Details Module Module has completed and had " + UserCntr + " User Detail rows returned.");
		outputStream.println(new SimpleDateFormat("MM/dd/yyyy HH:mm:ss").format(new Date()) + "-> "
				+ "User Alias Module has completed and had " + UACntr + " User Alias rows returned.");
		outputStream.flush();
...
// CREATE DATABASE CONNECTION
		// write to users_log file
				outputStream.println("## Create Database Connection nModule Log File ##");
				outputStream.flush();
...
//CLEANUP DATABASE
		...
		try {
			...
			deletestatement.executeUpdate(sql1);
			deletestatement.executeUpdate(sql2);
...
//INSERT USERS MODULE
		// write to users_log file
				outputStream.println("## Insert Users Module Log File ##");
				outputStream.flush();
		...
		// write to log file
		outputStream.println(new SimpleDateFormat("MM/dd/yyyy HH:mm:ss").format(new Date()) + "-> "
				+ "Insert Users into the Audit database completed and had "+UserDetailsSuccessfulInsert+" User Details rows inserted." );
		outputStream.println(new SimpleDateFormat("MM/dd/yyyy HH:mm:ss").format(new Date()) + "-> "
				+ "Insert Users into the Audit database completed and had "+UserAliasesSuccessfulInsert+" User Details rows inserted." );
		outputStream.flush();
